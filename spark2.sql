...
val percent = resultpercent.groupBy(resultpercent("query")).agg(avg(when(resultpercent("sumpercent").isNull, 0).otherwise(resultpercent("sumpercent"))).alias("percent")).orderBy(desc("percent")).limit(10).show()